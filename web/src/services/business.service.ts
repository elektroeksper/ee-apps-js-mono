    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400) {
          ...
          return {
            success: false,
            error: errorResult.error || 'Invalid business data',
          }
        }
        throw new Error(`HTTP error! status: ${response.status}`)
      }
  ...
  async getById(id: string): Promise<IOperationResult<IBusiness>> {
    try {
      ...
      if (result.success && result.data) {
        ...
        return {
          success: false,
          error: result.error || 'Failed to fetch business',
        }
      }
    } catch (error) {
  ...
  async verifyBusiness(
    ...
    try {
      ...
      if (result.success) {
        ...
        return {
          success: false,
          error: result.error || 'Failed to verify business',
        }
      }
    } catch (error) {
  ...
  async approveBusiness(
    ...
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/verification/${businessId}`,
        {
          method: 'PUT',
          headers,
          credentials: 'include', // Include session cookies as fallback
      ...
      if (result.success && result.data) {
        ...
        return {
          success: false,
          error: result.error || 'Failed to approve business',
        }
      }
    } catch (error) {
  ...
  async rejectBusiness(
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        return {
          success: false,
          error: result.error || 'Failed to reject business',
        }
      }
    } catch (error) {
  ...
  async clearBusinessRejection(
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        return {
          success: false,
          error: result.error || 'Failed to clear business rejection',
        }
      }
    } catch (error) {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to clear business rejection',
      }
    }
  }
}
