...
export interface UseUserProfileReturn extends UseUserProfileState, UseUserProfileActions { }
...
export function useUserProfile(userId?: string): UseUserProfileReturn {
  ...
  const updateUser = useCallback(async (id: string, data: Partial<IAppUser>): Promise<boolean> => {
    if (!id) {
      setError('User ID is required');
      return false;
    }
...
export function useBusinessOperations() {
  ...
  const clearRejection = useCallback(async (businessId: string): Promise<IBusiness | null> => {
    if (!businessId) {
