...
export function useFirebaseAuth(): FirebaseAuthState {
  ...
  if (!initialized.current) {
    ...
    console.log(
      '🔄 useFirebaseAuth: Direct initialization (React 19 compatible)...'
    )
    ...
    ;(async () => {
      try {
        ...
        if (!auth) {
          console.warn(
            '🚨 useFirebaseAuth: Firebase Auth not available (likely SSR)'
          )
        ...
        try {
          ...
          if (redirectResult.success && redirectResult.data) {
            console.log(
              '🔄 useFirebaseAuth: Redirect result found:',
              redirectResult.data.email
            )
            // The onAuthStateChanged will handle the user update
          }
        } catch (redirectError) {
          console.warn(
            '🚨 useFirebaseAuth: Error handling redirect result:',
            redirectError
          )
        ...
        console.log(
          '🔄 useFirebaseAuth: Current user on init:',
        ...
        onAuthStateChanged(auth, async user => {
          console.log(
            '🔄 useFirebaseAuth: Auth state changed to:',
            user?.email || 'null'
          )
          ...
          try {
            ...
            if (user) {
              console.log(
                '🔄 useFirebaseAuth: Setting user and loading claims...'
              )
              setFireUser(user)
