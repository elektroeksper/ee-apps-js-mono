...
export function useBusinessDocuments(): UseBusinessDocumentsState {
  ...
  const uploadDocument = useCallback(
    async (
      ...
      try {
        const result = await businessDocumentService.uploadDocument(
          file,
          category,
          metadata
        )
    ...
    async (documentId: string): Promise<boolean> => {
      ...
      try {
        ...
        if (result.success) {
          ...
          setDocuments(prev =>
            prev.filter(doc => doc.metadata?.documentId !== documentId)
          )
          return true
        } else {
    ...
    (file: File): { isValid: boolean; error?: string } => {
      ...
      if (!allowedTypes.includes(file.type)) {
        return {
          isValid: false,
          error:
            'Sadece PDF ve resim dosyaları (JPEG, PNG, GIF, WebP) kabul edilir',
        }
      }
