...
export async function verifyIdToken(token: string) {
  try {
    ...
    try {
      ...
      console.log('✅ Session cookie verified successfully for user:', decodedToken.email);
      console.log('🔍 Session cookie claims:', decodedToken.admin, decodedToken.role);
      ...
      console.log('✅ ID token verified successfully for user:', decodedToken.email);
      console.log('🔍 ID token claims:', decodedToken.admin, decodedToken.role);
      return { success: true, user: decodedToken };
    }
  } catch (error) {
    console.error('❌ Error verifying token (both session and ID token failed):', error);
