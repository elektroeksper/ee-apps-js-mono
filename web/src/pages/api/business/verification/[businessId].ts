...
async function handleUpdateVerificationStatus(
  ...
  try {
    ...
    if (!action || !['approve', 'reject', 'clear-rejection'].includes(action)) {
      return res.status(400).json({ error: 'Valid action is required (approve, reject, clear-rejection)' });
    }
    ...
    const currentVerification = business?.verification || { status: BusinessVerificationStatus.PENDING, history: [] };
    ...
    return res.status(500).json({ error: 'Failed to update business verification status' });
  }
}