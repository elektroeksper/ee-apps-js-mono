  ...
  try {
    ...
    if (sessionCookie) {
      try {
        ...
        console.log(
          '🔑 Business API: Session cookie verification failed:',
    ...
    if (!decodedToken) {
      ...
      if (authHeader?.startsWith('Bearer ')) {
        ...
        try {
          ...
          console.log(
            '🔑 Business API: ID token verification failed:',
    ...
    if (!decodedToken || !userId) {
      console.log(
        '❌ Business API: Authentication failed - no valid session or ID token'
      )
    ...
    if (
      ...
      console.log(
        '❌ No permission to fetch documents. isAdmin:',
        isAdmin,
        'ownerId:',
...
async function handleUpdateBusiness(
  ...
  try {
    ...
    if (!isOwner && !hasEditPermission) {
      return res
        .status(403)
        .json({ error: 'Insufficient permissions to edit business' })
    }
...
async function handleDeleteBusiness(
  ...
  try {
    ...
    if (!isAdmin && !isOwner && !hasDeletePermission) {
      return res
        .status(403)
        .json({ error: 'Insufficient permissions to delete business' })
    }
