...
async function handleGetBusinesses(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const verificationStatus = req.query
      .verificationStatus as BusinessVerificationStatus
    ...
    let businesses: IBusiness[] = snapshot.docs.map(
      (doc: any) =>
        ({
          id: doc.id,
          ...doc.data(),
...
async function handleCreateBusiness(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (userData?.businessInfo?.businessId) {
      return res
        .status(400)
        .json({ error: 'User already has a business associated' })
    }
