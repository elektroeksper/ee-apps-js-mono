...
export default async function handler(
  ...
  try {
    ...
    const isAdminFromCustomClaims = userClaims.admin === true || userClaims.role === 'admin';
    const isAdminFromDirect = decodedResult.user.admin === true || decodedResult.user.role === 'admin';
    ...
    const businessUsers = users.filter(user => user.accountType === AccountType.BUSINESS).length;
    const individualUsers = users.filter(user => user.accountType === AccountType.INDIVIDUAL).length;
    ...
    const businessVerificationStats = {
      pending: businesses.filter(b => b.verification?.status === BusinessVerificationStatus.PENDING).length,
      verified: businesses.filter(b => b.verification?.status === BusinessVerificationStatus.VERIFIED).length,
      rejected: businesses.filter(b => b.verification?.status === BusinessVerificationStatus.REJECTED).length,
      unverified: businesses.filter(b => b.verification?.status === BusinessVerificationStatus.UNVERIFIED).length,
    };
