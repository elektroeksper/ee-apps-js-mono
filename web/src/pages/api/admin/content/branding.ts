...
async function handleGetBrandingInfo(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch branding information' });
  }
}

async function handleUpdateBrandingInfo(req: NextApiRequest, res: NextApiResponse) {
  try {
    const user = await verifyAdminAuth(req);
    const brandingData = req.body;

    if (!brandingData || !brandingData.businessName || !brandingData.logoUrl) {
      return res.status(400).json({ error: 'Business name and logo URL are required' });
    }
    ...
    return res.status(500).json({ error: 'Failed to update branding information' });
  }
}