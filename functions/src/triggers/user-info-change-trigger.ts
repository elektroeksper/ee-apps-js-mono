...
export const onUserProfileUpdate = onDocumentUpdated('users/{userId}', async (event) => {
  ...
  if (!beforeData || !afterData) {
    functions.logger.warn('Missing before or after data in user update trigger');
    return;
  }
  ...
  if (!businessInfo || !businessInfo.businessId) {
    functions.logger.warn('no business info or businessId found for user', { userId });
  ...
  try {
    ...
    try {
      ...
      functions.logger.warn(`Failed to get Auth record for user ${userId}:`, err);
    }
    ...
    if (currentUsers[userId]) {
      ...
      functions.logger.warn(`User ${userId} not found in business ${businessId} users list`);
    }
  } catch (error) {
