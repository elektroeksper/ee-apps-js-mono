...
export class BusinessService {
  ...
  async getAll(filter?: IBusinessFilter): Promise<IOperationResult<IBusiness[]>> {
    try {
      let query: FirebaseFirestore.Query | FirebaseFirestore.CollectionReference = db.collection(this.collectionName);
  ...
  async create(data: IBusiness): Promise<IOperationResult<IBusiness>> {
    try {
      ...
      const createdBusiness = { ...createdDoc.data() as IBusiness, id: docRef.id };
  ...
  async verifyBusiness(id: string, approve: boolean, adminId: string, reason?: string): Promise<IOperationResult<void>> {
    try {
      ...
      const currentVerification = currentData.verification || { status: BusinessVerificationStatus.UNVERIFIED, history: [] };

      const verificationUpdate = {
        status: approve ? BusinessVerificationStatus.VERIFIED : BusinessVerificationStatus.REJECTED,
        history: [
  ...
  async addUser(businessId: string, userId: string, info: IBusinessUserInfo): Promise<IOperationResult<void>> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to add user to business'
      };
    }
  }
  ...
  async removeUser(businessId: string, userId: string): Promise<IOperationResult<void>> {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to remove user from business'
      };
    }
  }
