...
async function testEmailConfig() {
  ...
  console.log(
    `   SMTP_PASS: ${env.SMTP_PASS ? `${env.SMTP_PASS.substring(0, 4)}****` : 'NOT SET'}`
  )
  ...
  if (env.SMTP_PASS === 'REPLACE_WITH_NEW_APP_PASSWORD') {
    ...
    console.log(
      '📋 Please update SMTP_PASS in .env file with your Gmail App Password'
    )
    process.exit(1)
  }
  ...
  if (env.SMTP_PASS && env.SMTP_PASS.includes(' ')) {
    console.warn(
      '⚠️  WARNING: SMTP_PASS contains spaces - this might cause issues'
    )
...
This email was sent from your local test script.
    ...
    if (error.code === 'EAUTH') {
      ...
      console.log(
        '4. Try creating App Password with "Other (custom)" and name it "Firebase SMTP"'
      )
