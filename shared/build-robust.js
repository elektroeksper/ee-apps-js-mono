...
function validateSourceFiles() {
  ...
  const sourceFiles = fs.readdirSync(SRC_DIR, { recursive: true })
...
function buildWithExplicitFlags() {
  ...
  try {
    ...
    try {
      execSync('npx tsc src/index.ts --outDir ./dist --declaration --target ES2020 --module commonjs --moduleResolution node --allowJs --skipLibCheck --esModuleInterop', {
        cwd: SHARED_DIR,
        stdio: 'pipe'
      });
    } catch (indexError) {
  ...
  try {
    ...
    for (const file of tsFiles) {
      ...
      try {
        ...
        execSync(`npx tsc "${file}" --outDir "${DIST_DIR}" --declaration --target ES2020 --module commonjs --moduleResolution node --allowJs --skipLibCheck --esModuleInterop`, {
          cwd: SHARED_DIR,
          stdio: 'pipe'
        });
      } catch (fileError) {
...
function validateBuild() {
  ...
  log.success(`Build validation passed: ${jsFiles.length} JS files, ${dtsFiles.length} declaration files`);
